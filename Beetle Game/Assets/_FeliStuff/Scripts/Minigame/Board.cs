...
{
    //HACK bullshit for gate and tests
...
[SerializeField]
    ...
    private void BoardSetup(eAbility ability)
    {
        tempAbility = ability;
    ...
    public void GameResult(bool win)
    {
        if (win)
        {
            ...
            AbilityManager.Instance.SetPassiveAbility(Color.blue);
        }
        else
        {
            ChangeGameState(eMinigameState.Lose);
            AbilityManager.Instance.SetPassiveAbility(Color.white);
        }
